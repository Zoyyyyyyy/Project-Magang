...
export function renderPromoTabs(promoData) {
  /**
   * Extract caption text from a raw field (string or object).
   */
  function extractCaption(raw) {
    if (!raw) return '';
    if (typeof raw === 'string') return raw;
    if (typeof raw === 'object') return raw.caption || raw.teks || raw.text || '';
    return String(raw);
  }

  /**
   * Render a single platform's content card into its tab panel element.
   * @param {string} key  - 'wa' | 'ig' | 'fb' | 'tiktok'
   * @param {any}    rawData
   */
  function renderCard(key, rawData) {
    const el = document.getElementById('tab-content-' + key);
    if (!el) return;

    // Normalise: parse if JSON string
    let data = rawData;
    ...
    if (!data || typeof data !== 'object') {
      el.innerHTML = `
        <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm">
          <div class="whitespace-pre-wrap text-sm text-slate-700 leading-relaxed">${escHtml(String(data ?? ''))}</div>
        </div>`;
      return;
    }

    const caption    = data.caption    || data.teks      || data.text     || '';
    const ideKonten  = data.visual     || data.ide_konten                 || '';
    const langkahRaw = data.langkah;

    // Normalise langkah → always an array
    let langkahItems = [];
    if (Array.isArray(langkahRaw)) {
      langkahItems = langkahRaw;
    } else if (typeof langkahRaw === 'string' && langkahRaw.trim()) {
      // Split on " | " separator (legacy format)
      langkahItems = langkahRaw.split('|').map(s => s.trim()).filter(Boolean);
    }

    // Numbered list HTML
    const langkahHtml = langkahItems.map((step, i) => `
      <li class="flex items-start gap-2.5 text-sm text-slate-700">
        <span class="flex-shrink-0 w-5 h-5 rounded-full bg-brand-500 text-white text-xs font-bold flex items-center justify-center mt-0.5">${i + 1}</span>
        <span class="leading-relaxed">${escHtml(String(step))}</span>
      </li>
    `).join('');
    ...
    el.innerHTML = `
      <div class="flex flex-col gap-4 h-auto">
        ...
        <!-- Caption -->
        <div class="bg-white border border-slate-200 rounded-xl p-4 shadow-sm h-auto">
          <h3 class="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">📝 Teks Caption</h3>
          <p class="whitespace-pre-wrap text-sm text-slate-800 leading-relaxed font-medium">${escHtml(caption)}</p>
        </div>

        ${ideKonten ? `
        <!-- Ide Visual -->
        <div class="bg-amber-50 border border-amber-100 rounded-xl p-4 shadow-sm h-auto">
          <h3 class="text-xs font-bold text-amber-600 uppercase tracking-wider mb-2">💡 Ide Konten &amp; Visual</h3>
          <p class="text-sm text-slate-700 leading-relaxed italic">${escHtml(ideKonten)}</p>
        </div>` : ''}

        ${langkahItems.length > 0 ? `
        <!-- Langkah -->
        <div class="bg-brand-50 border border-brand-100 rounded-xl p-4 shadow-sm h-auto">
          <h3 class="text-xs font-bold text-brand-600 uppercase tracking-wider mb-3">📌 Langkah Pembuatan</h3>
          <ol class="flex flex-col gap-2">${langkahHtml}</ol>
        </div>` : ''}
  ...
  renderCard('wa',     promoData.whatsapp);
  renderCard('ig',     promoData.instagram);
  renderCard('fb',     promoData.facebook);
  renderCard('tiktok', promoData.tiktok);
}
